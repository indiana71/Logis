<br><br>
<?php $result = $mysqli->query("SELECT distinct libri.*,autori.*,scritto.isbn,editori.NomeE as nomeed, immagini.* FROM libri,scritto,autori,editori, immagini
		WHERE 
		(autori.idautore=scritto.idautore and scritto.isbn=libri.isbn) 
		and(editori.ideditore=libri.ideditore)and(immagini.isbn = libri.isbn)ORDER BY Sezione");
            ...
            <div class="card-body">
                <div class="card-content"> 
					<article class="bcp-block-item-issue clearfix block-item-post list">
						...
						<h5 class="block-sub-title" style="color : blue;"> <?php echo  $row['Titolo'] ?> </h5>
						<br><span class="block-info block-info--author">ISBN: <?php echo  $row['isbn'] ?></span>
						<br><span class="block-info block-info--author">Autore: <?php echo  $row['CognomeA'].' '.$row['NomeA'] ?></span>
						<br><span class="block-info block-info--author">Editore: <?php echo  $row['nomeed'] ?></span>
						<br><span class="block-info block-info--author" style="color : blue;">Sezione: <?php echo  $row['Sezione'] ?></span>
